window.onload=function(){
    ...
    var aspanProduct=oProduct.getElementsByTagName('span');
    for(var i=0;i<aimgProduct.length;i++){
        aliProduct[i].index=i;
        aliProduct[i].onclick=function(){
            var big="big";
            var numberProduct="numberProduct";
            var numberSpan=aspanProduct[this.index].innerHTML;
            var aUrl=aimgProduct[this.index].src;
            document.cookie=big+'='+aUrl;
            document.cookie=numberProduct+'='+numberSpan;
        }
}
//弹出登陆框并加遮罩层
